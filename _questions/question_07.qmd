...
analysis_df |>
  ...
  geom_point(alpha = 0.5, aes(size = population)) +
  geom_smooth(
...
table_continent_year <- analysis_df |>
  ...
  summarise(
    avg_gdp_pcap_pp = weighted.mean(gdp_pcap_pp, w = population, na.rm = TRUE),
    avg_urban_pct   = weighted.mean(urban_pct, w = population, na.rm = TRUE),
